    ...
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('invoiceUpdate', ['id' => $data['current']['id']]) }}">
                        @csrf
                        <div class="form-group row">
                            ...
                            <div class="col-md-6">
                                <select name="id" class="form-control">
                                    ...
                                    <option value="{{ $invoice->id }}" {{ $invoice->id == $data['current']['id'] ? 'selected' : '' }}>{{ $invoice->id }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="form-group row mb-0">
                            <div class="col-md-6 offset-md-4">
                                ...
                                <a class="btn btn-danger" href="{{ route('invoiceDelete',['id' => $data['current']['id']]) }}">{{ __('Delete') }}</a>
                            </div>
                        </div>
                    </form>
                </div>
...
@section('custom-js-btm')
    <script>
        $(document).ready(function() {
            $('select').select2({ width: 'resolve' });
            console.log("im ready")
            $('[name = id]').on('select2:select', function(e){
                window.location = 'http://localhost:8000/invoice/edit/'+e.params.data.id
            })
        });
